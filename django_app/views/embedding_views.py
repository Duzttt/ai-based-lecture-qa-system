...
def list_embedding_models(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        cache_stats = embedding.get_cache_stats()
        return JsonResponse({"models": models, "cache_stats": cache_stats})
...
def get_current_embedding_model(request: HttpRequest) -> JsonResponse:
    ...
    try:
        current_id = embedding.get_current_model_id()
        model_info = embedding.AVAILABLE_MODELS.get(current_id, {})
        cache_stats = embedding.get_cache_stats()
...
def switch_embedding_model(request: HttpRequest) -> JsonResponse:
    ...
    try:
        if model_id not in embedding.AVAILABLE_MODELS:
            return _error_response(f"Unknown model: {model_id}", status=400)

        result = embedding.set_current_model(model_id)
...
def test_embedding_model(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        result = embedding.test_model(model_id, query, top_k=top_k)
        return JsonResponse({"success": True, **result})
    except Exception as exc:
...
def get_embedding_model_metrics(request: HttpRequest) -> JsonResponse:
    ...
    try:
        metrics = embedding.get_performance_metrics(limit=50)

        model_stats: Dict[str, Dict[str, Any]] = {}
        for metric in metrics:
            mid = metric["model_id"]
            if mid not in model_stats:
                model_stats[mid] = {"count": 0, "total_time_ms": 0, "actions": {}}

            model_stats[mid]["count"] += 1
            model_stats[mid]["total_time_ms"] += metric["time_ms"]

            action = metric["action"]
            if action not in model_stats[mid]["actions"]:
                model_stats[mid]["actions"][action] = {"count": 0, "total_time_ms": 0}
            model_stats[mid]["actions"][action]["count"] += 1
            model_stats[mid]["actions"][action]["total_time_ms"] += metric["time_ms"]

        for mid, stats in model_stats.items():
            stats["avg_time_ms"] = (
                round(stats["total_time_ms"] / stats["count"], 2) if stats["count"] > 0 else 0
            )
...
def clear_embedding_model_cache(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return JsonResponse({"success": True, "message": "Model cache cleared"})
    except Exception as exc:
