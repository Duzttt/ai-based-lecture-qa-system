...
const handleFileUpload = async (event) => {
  ...
  try {
    await uploadPDF(file, (progress) => { uploadProgress.value = progress })
    await loadFiles()
...
const toggleDocSelection = (docName) => { documentStore.toggleDocSelection(docName) }
...
<template>
  <div class="panel sources-panel">
    <div class="panel-header">
      ...
      <button class="select-all-btn" @click="toggleSelectAll" :disabled="sources.length === 0">
    ...
    <div class="panel-body">
      ...
      <div v-if="sources.length === 0" class="sources-empty">
        <svg class="empty-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 4h5v8l-2.5-1.5L6 12V4z"/></svg>
      ...
      <div v-else class="sources-list">
        ...
        >
          <svg class="source-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>
          <span class="source-name" :title="source.name || source.filename">
            {{ source.name || source.filename }}
          </span>
          <button class="source-remove" @click.stop="removeFile(source.id)" title="Remove">
    ...
    <div class="panel-footer">
      <button class="footer-btn" @click="showUploadModal = true">
        <svg class="footer-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"/></svg>
        Manage Sources
      </button>
      <button class="footer-btn-icon" title="Settings">
    ...
    <div v-if="showUploadModal" class="upload-overlay" @click.self="showUploadModal = false">
      <div class="upload-modal" @drop="handleDrop" @dragover="handleDragOver">
        <div class="upload-header">
          <h3>Upload PDF</h3>
          <button class="upload-close" @click="showUploadModal = false">
            <svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
          </button>
        </div>
        <div class="upload-body">
          <label class="upload-area">
            <input type="file" accept=".pdf" @change="handleFileUpload" :disabled="uploading" hidden />
