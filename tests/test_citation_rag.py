        ...
        with patch("app.services.citation_rag.EmbeddingService"):
            with patch("app.services.citation_rag.VectorStore"):
                ...
                chunks = [
                    {
                        "chunk_id": 1,
                        "text": "Agents operate autonomously.",
                        "source": "L1.pdf",
                        "page": 24,
                    },
                    {
                        "chunk_id": 2,
                        "text": "They can reason and choose actions.",
                        "source": "L2.pdf",
                        "page": 3,
                    },
                ]
    ...
    def test_parse_invalid_citations_type_raises_error(self):
        ...
        with patch("app.services.citation_rag.EmbeddingService"):
            with patch("app.services.citation_rag.VectorStore"):
                ...
                with pytest.raises(
                    CitationRAGError, match="citations must be an array"
                ):
                    pipeline._parse_llm_response(raw_response)
...
class TestMalformedJsonFallback:
    """Test fallback behavior for malformed JSON responses."""

    def test_malformed_json_returns_user_friendly_message(self):
        """Test that malformed JSON like '{\"sentences\": [' returns error message."""
        with patch("app.services.citation_rag.EmbeddingService") as mock_embedding:
            with patch("app.services.citation_rag.VectorStore") as mock_vector_store:
                mock_embedding_instance = Mock()
                mock_embedding_instance.embed_query.return_value = [0.1] * 384
                ...
                with patch.object(pipeline, "_generate") as mock_generate:
                    ...
                    assert "sentences" in result
                    assert len(result["sentences"]) == 1
                    assert "malformed" in result["sentences"][0]["text"].lower()
                    assert "rephrasing" in result["sentences"][0]["text"].lower()

    def test_plain_text_fallback_still_works(self):
        """Test that valid plain text (not JSON-like) still returns as-is."""
        with patch("app.services.citation_rag.EmbeddingService") as mock_embedding:
            with patch("app.services.citation_rag.VectorStore") as mock_vector_store:
                mock_embedding_instance = Mock()
                mock_embedding_instance.embed_query.return_value = [0.1] * 384
                ...
                # Simulate LLM returning plain text (not JSON-like)
                with patch.object(pipeline, "_generate") as mock_generate:
                    mock_generate.return_value = (
                        "The answer is that agents operate autonomously."
                    )
...
class TestQueryWithCitationsFunction:
    ...
    def test_query_with_citations_calls_pipeline(self):
        ...
        with patch(
            "app.services.citation_rag.CitationRAGPipeline"
